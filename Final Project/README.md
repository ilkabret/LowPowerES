...
| Component | Reference |
|-----------|-----------|
| 🖥️ MCU board | Arduino Nano 33 BLE Sense Rev 2 (Nordic nRF52840, Cortex-M4F @ 64 MHz, 256 KB SRAM, 1 MB Flash) |
...
> ⚠️ The on-board camera/microphone are **not** used. Inputs are streamed from the host over USB-serial or pre-loaded as C arrays in flash.
...
2. ▶️ Press the user button to start the inference loop.
...
- [ ] 🗜️ Stage 2 — compression sweep complete (PTQ, pruning, distillation, combinations)
- [ ] 📟 Stage 3 — all variants deployed on Arduino with measured latency / memory / energy
- [ ] ⚙️ Stage 4 — adaptive runtime policy implemented and validated on battery
